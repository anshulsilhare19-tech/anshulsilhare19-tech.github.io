document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('product-detail-content')) {
        ...
        if (product) {
            ...
            `;
            attachAddToCartListeners(); // Re-attach event listeners for the new button
        } else {
    ...
    // --- Order History Logic ---
    if (document.getElementById('order-history-content')) {
        renderOrderHistory();
    }
    ...
    function renderOrderHistory() {
        ...
        if (orders.length > 0) {
            let historyHtml = '';
            orders.reverse().forEach(order => { // .reverse() shows newest orders first
                historyHtml += `
            ...
            historyContainer.innerHTML = '<p>You have no past orders.</p>';
        }
    }
    
    // --- Checkout Form Logic (RESTORED) ---
    const checkoutForm = document.getElementById('checkout-form');
    if (checkoutForm) {
        checkoutForm.addEventListener('submit', function(e) {
            // GTM event for begin_checkout would go here if needed
        });
    }
    ...
    if (window.location.pathname.endsWith('thankyou.html')) {
        ...
        if (cart.length > 0) {
            ...
            // GTM purchase event would go here
            
            localStorage.removeItem('cart'); // Clear cart after processing order
        }
    }
